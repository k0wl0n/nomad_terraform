...
readonly PLUGIN_CONFIG_FILE="plugin.hcl"
...
function generate_nomad_config {
  ...
  local readonly plugin_path="$config_dir/$PLUGIN_CONFIG_FILE"
...
EOF
  ...
  log_info "Creating default Nomad plugin config file in $plugin_path"
  cat > "$plugin_path" <<EOF
plugin "docker" {
  config {
...
{
    "credHelpers": {
    "866275565887.dkr.ecr.ap-southeast-1.amazonaws.com": "ecr-login"
    }
}
EOF

  chown "$user:$user" "$config_path"
  chown "$user:$user" "$plugin_path"
}
